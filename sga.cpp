...
int decodeY(int index)
{
  ...
  for (int i=CHROM_LENGTH/2; i<CHROM_LENGTH; i++)
    value += (int) pow(2.0,(double)(i-(CHROM_LENGTH/2))) * pool[index].string[CHROM_LENGTH-1-i];
  return (value);
}
...
double evaluate(int valueX, int valueY)
{
  // cout << valueX << " " << convRange(valueX) << "\n";
  // cout << valueY << " " << convRange(valueY) << "\n";
...
void statistics()
{
  ...
  for (i=0; i<POPULATION_SIZE; i++)
  {
    ...
    cout << "\t" << convRange(decodeX(i));
    cout << "\t" << convRange(decodeY(i));
  }
  cout << endl;
}
